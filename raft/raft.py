...
class RaftNode(threading.Thread):
    def __init__(self, config, name, role='follower', verbose=True, callback=None):
        ...
        self.election_timeout = random.uniform(
            TMOUT, TMOUT+TMOUT*len(self.all_ids)*0.7
            )  # Failed vote backoff, used for pretty much all timing related things.
        self.heartbeat_frequency = TMOUT                                    # How often to send heartbeat (should be less than the election timeout).
        self.resend_time = RESEND                                           # How often to resend an append entries if you havent heard from a node in a while.
    ...
    def pause(self):
        ...
        if (self.verbose):
            log.warning(self._name + ': pausing...')
    ...
    def un_pause(self):
        ...
        if (self.check_role() == 'none'):
            ...
            if (self.verbose):
                log.warning(self._name + ': unpausing...')
        else:
            if (self.verbose):
                log.warning(self._name + ': node was not paused, doing nothing')
    ...
    def run(self):
        ...
        try:
            while not self._terminate:
                ...
                if role == 'leader':
                    candidate_count = 0
                    # if self.callback and state != 'leader':
                    #     state = 'leader'
                    #     self.callback('leader')
                    self._leader()
                elif role == 'follower':
                    candidate_count = 0
                    # if self.callback and \
                    #    (self.leader_id is None or state != 'follower'):
                    #     state = 'follower'
                    ...
                    if candidate_count > max_candidate_count:
                        ...
                        # if self.callback and state != 'candidate':
                        #     state = 'candidate'
    ...
    def _candidate(self):
        ...
        if (self.verbose):
            log.warning(self._name + ': became candidate')
        ...
        while ((not self._terminate) and (self.check_role() == 'candidate')):
            ...
            if ((time.time() - time_election_going) > self.election_timeout):
                if (self.verbose):
                    log.warning(self._name + ': election timed out')
                self._set_current_role('candidate')
                return
    ...
    def _leader(self):
        ...
        if (self.verbose):
            log.warning(self._name + ': became leader')
                ...
                if (incoming_message.direction == MessageDirection.Response):
                    ...
                    if (incoming_message.type == MessageType.Acknowledge):
                        ...
                        if (self.next_index[sender_index] is not None):
                            ...
                            if (self.verbose):
                                log.warning(self._name + ": updated standing is " + str(self.match_index) + " my index: " + str(self._log_max_index()))
                ...
                # Handle incoming requests
                    ...
                    if (incoming_message.type == MessageType.RequestVotes):
                        if (incoming_message.term > self.current_term):
                            ...
                            if (self.verbose):
                                log.warning(self._name + ': saw higher term, demoting')
                            return
    ...
    def _set_current_role(self, role):
        ...
        # log.debug(f"** {old_role} -> {role}")

        with self.client_lock:
            self.current_role = role

        if (old_role != role) or \
           (self.leader_id is None and old_role == 'follower'):
            t = Thread(target=self.callback, args=[self, role])
            t.start()
...
def test_failures():
    ...
    leader = [n for n in s if (n.check_role() == 'leader')][0]
    leader.pause()
